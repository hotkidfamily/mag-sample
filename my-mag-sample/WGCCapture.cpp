...
bool WGCCapture::onCaptured(D3D11_MAPPED_SUBRESOURCE &rect, D3D11_TEXTURE2D_DESC &header)
{
    ...
    if (!_frames.get() || width != _frames->width() || height != _frames->height()
...
void WGCCapture::_run()
{
    ...
    if (_hwnd != nullptr) {
        winrt::check_hresult(interopFactory->CreateForWindow(_hwnd, winrt::guid_of<ABI::Windows::Graphics::Capture::IGraphicsCaptureItem>(),
                                        reinterpret_cast<void **>(winrt::put_abi(captureItem))));
    }
    else {
        winrt::check_hresult(interopFactory->CreateForMonitor(_hmonitor, winrt::guid_of<ABI::Windows::Graphics::Capture::IGraphicsCaptureItem>(),
                                    reinterpret_cast<void **>(winrt::put_abi(captureItem))));
    }
